...
DnssdErrorType DnssdClient::RegisterDnssdService(const char* service_name,  const char* service_type, const char* domain, const char* host_name, uint16_t network_port, const char* txt_record, uint16_t txt_record_length, DnssdRegisterCallback callback_function, void* user_data )
{
    // create a dns service 
//    DnssdErrorType result = mDnssdCreateServiceFunc(serviceName.c_str(), port.c_str(), &mDnssdServicePtr);
    DnssdErrorType result = DNSSD_NO_ERROR;
	result = mDnssdRegisterServiceFunc(service_name, service_type, domain, host_name, network_port, txt_record, txt_record_length, callback_function, user_data, &mDnssdServicePtr);
